...
//                    assert(vehicle->getPartner() == nullptr);
    ...
    std::map<std::string, std::pair<std::pair<int, int>, std::pair<int,int>>> Engine::getVehicleLocation() const {
        std::map<std::string, std::pair<std::pair<int, int>, std::pair<int,int>>> ret;
        for (const Vehicle* vehicle : getRunningVehicles()) {
            auto curpos = vehicle->getCurPos();
            std::pair<int, int> pos1, pos2;
            pos1.first = curpos.first.x;
            pos1.second = curpos.first.y;
            pos2.first = curpos.second.x;
            pos2.second = curpos.second.y;

            ret.emplace(vehicle->getId(), std::pair<std::pair<int, int>, std::pair<int,int>>(pos1, pos2));
        }
        return ret;
    }
